...
- **アプリシェル:** 左固定サイドバーは**展開 `w-64` / 折りたたみ `w-20`**（`transition-all duration-300`、`motion-reduce:transition-none`）。右枠線 `border-r border-zinc-100/50`。状態は `localStorage` キー **`rikai.sidebarOpen`**（`"1"` 展開 / `"0"` 折りたたみ）。旧キー `rikai.sidebarCollapsed` は初回のみ読み取り移行。メインは `flex-1`。サイドバーは `sticky top-0 h-screen`、`backdrop-blur` 付きの半透明（`bg-white/50 backdrop-blur-md`）。
- **メインコンテンツエリア:** `p-16`、`max-w-5xl`、`mx-auto`（`__root.tsx` の `AppShell`）。新規画面も原則この枠に収める。
- **サイドバー内:** ロゴ行 `h-24`・`px-6`、ヘッダー右に **PanelLeftClose / PanelLeftOpen** のトグル。ナビは `px-4 py-4`、`px-4 py-3` の項目。ラベルは幅開閉で `opacity` / `w` をアニメーション。
...
- アイコン＋ラベル横並び、`text-sm font-bold`。折りたたみ時はラベルを畳み、**`title` / `aria-label`** で補足。
- ロゴ「Rikai」は展開時のみ表示。折りたたみ時は `pointer-events-none`・`tabIndex={-1}`・`aria-hidden` でヘッダーリンクを隠し、ホームはナビの「ロードマップ」から辿る想定。

### 6.2 プラン表示（サイドバー下部）

- 展開時のみ表示（`p-6` ラッパー）。折りたたみ時は `h-0 overflow-hidden opacity-0` で非表示。
- カード: `bg-emerald-50/50`、`border-emerald-100/50`、`rounded-2xl`、`p-4` 前後。キャプションは極小＋`uppercase tracking-widest`

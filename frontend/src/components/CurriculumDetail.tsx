...
export const CurriculumDetail: React.FC<DetailProps> = ({ curriculum, onUpdateTask, onSetTaskContent }) => {
  const [selectedTask, setSelectedTask] = useState<Task | null>(curriculum.modules[0]?.tasks[0] || null);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const response = await getLearningSupport(curriculum, selectedTask.title, inputMessage, chatMessages);
  ...
  const formatText = (text: string) => {
    return text.split('\n').map((line, i) => (
      <span key={i}>
        {line.split('**').map((part, j) => j % 2 === 1 ? <strong key={j} className="text-slate-900 font-bold">{part}</strong> : part)}
        <br />
      </span>
    ));
  };
    ...
    <div className="fixed inset-0 bg-white flex flex-col animate-in fade-in duration-500">
      ...
      <header className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-white z-20">
        <div className="flex items-center gap-4">
          <Link to="/" className="p-2 hover:bg-slate-50 rounded-lg text-slate-400 hover:text-indigo-600 transition-all">
            <ChevronLeft size={24} />
          </Link>
          <div className="h-6 w-px bg-slate-100 mx-2"></div>
          <div>
            <h1 className="text-sm font-black text-slate-900 truncate max-w-[200px] md:max-w-md">{curriculum.title}</h1>
            <div className="flex items-center gap-3 mt-0.5">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{calculateOverallProgress()}% Complete</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* Slim Sidebar */}
        <aside className={`${isSidebarOpen ? 'w-80' : 'w-0'} bg-slate-50 border-r border-slate-100 flex flex-col transition-all duration-300 overflow-hidden relative z-10`}>
          <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-6">
            {curriculum.modules.map((module, mIdx) => (
              <div key={module.id} className="space-y-1">
                <div className="px-3 py-2 flex items-center justify-between">
                  <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Section {mIdx + 1}</h2>
                </div>
                <div className="space-y-1">
                  {module.tasks.map((task) => (
                    ...
                    >
                      <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${selectedTask?.id === task.id ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-100 group-hover:border-indigo-100'}`}>
          ...
          <div className="max-w-3xl mx-auto px-8 py-16">
            {selectedTask ? (
                ...
                {isLoadingContent ? (
                  <div className="flex flex-col items-center justify-center py-32 text-center space-y-6">
                    <div className="relative">
                      ...
                      <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-200" size={24} />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-xl font-black text-slate-900">AIが講義資料を編集中...</h3>
                      <p className="text-slate-400 text-sm font-medium">トピックに最適化された解説とクイズを準備しています。</p>
                    </div>
                  </div>
                  ...
                  <article className="space-y-16">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 text-indigo-500">
                        <BookOpen size={16} />
                        <span className="text-[10px] font-black uppercase tracking-widest">Lesson Content</span>
                      </div>
                    ...
                    <section className="bg-slate-900 text-white rounded-[40px] p-8 md:p-12 shadow-2xl relative overflow-hidden group">
                      ...
                      <div className="relative z-10 space-y-8">
                        ...
                        <div className="space-y-8">
                          <p className="text-2xl font-bold leading-snug">{selectedTask.content.quiz.question}</p>
                          <div className="grid gap-3">
                            {selectedTask.content.quiz.options.map((option, idx) => (
                              ...
                              >
                                <span className={`w-10 h-10 rounded-xl border-2 border-current flex items-center justify-center text-sm transition-colors ${quizAnswer === idx ? 'bg-white text-slate-900' : ''}`}>
                          ...
                          {quizAnswer !== null && (
                            <div className="mt-10 animate-in zoom-in-95 duration-500 bg-white/10 p-8 rounded-[32px] border border-white/10 backdrop-blur-md">
                              <div className="flex items-center gap-4 mb-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${quizAnswer === selectedTask.content.quiz.correctAnswer ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                                  {quizAnswer === selectedTask.content.quiz.correctAnswer
                                    ? <CheckCircle size={28} className="text-white" />
                                    : <X size={28} className="text-white" />}
                                </div>
                                <span className={`text-2xl font-black ${quizAnswer === selectedTask.content.quiz.correctAnswer ? 'text-emerald-400' : 'text-rose-400'}`}>
                                  {quizAnswer === selectedTask.content.quiz.correctAnswer ? '正解です！' : '惜しい...'}
                                </span>
                              </div>
                              ...
                              {quizAnswer === selectedTask.content.quiz.correctAnswer && (
                                <button
                                  onClick={() => onUpdateTask(curriculum.id, selectedTask.id, TaskStatus.COMPLETED)}
                  ...
                  <div className="flex flex-col items-center justify-center py-32 text-center">
                    ...
                    <h3 className="text-xl font-black text-slate-800 tracking-tight">読み込みに失敗しました</h3>
                    <p className="text-slate-400 text-sm mt-2 mb-8">ネットワーク環境を確認して再試行してください。</p>
                    <button onClick={() => loadTaskDetails(selectedTask)} className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-black hover:bg-indigo-700 transition-all">
                      再試行する
                    </button>
                  </div>
                )}
              ...
              <div className="flex flex-col items-center justify-center py-32 text-center h-full">
                ...
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">学習を開始しましょう</h3>
        ...
        {/* AI Mentor Drawer */}
        <div className={`fixed inset-y-0 right-0 w-[420px] bg-white border-l border-slate-100 shadow-2xl z-30 flex flex-col transition-transform duration-500 ${showMentor ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-slate-900 text-white">
            ...
            <button onClick={() => setShowMentor(false)} className="p-2 hover:bg-white/10 rounded-lg text-white/50 hover:text-white transition-colors">
              <X size={24} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6 bg-slate-50/30">
            {chatMessages.length === 0 && (
              <div className="text-center py-16 opacity-40">
                ...
                <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Mental Support & Q&A</p>
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                <div className={`max-w-[85%] px-6 py-4 rounded-[28px] text-sm shadow-sm leading-relaxed ${
                  msg.role === 'user'
